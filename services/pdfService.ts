...
const FONT_URLS: Record<string, Record<string, string>> = {
    ...
    'dancing-script': {
        normal: 'https://raw.githubusercontent.com/google/fonts/main/ofl/dancingscript/DancingScript%5Bwght%5D.ttf',
        bold: 'https://raw.githubusercontent.com/google/fonts/main/ofl/dancingscript/DancingScript%5Bwght%5D.ttf'
    },
    'patrick-hand': {
  ...
  pageNodes.forEach((nodeId, index) => {
    ...
    sortedElements.forEach(el => {
      ...
      if (textContent) {
        ...
        // Vertical Alignment Logic for Text Box / Shapes
        if (el.verticalAlign === 'top') {
            posY = ly + 4; // Padding
            baseline = 'top';
            if (el.type === 'triangle') posY += h / 6; // Adjustment for triangle
        } else if (el.verticalAlign === 'bottom') {
            posY = ly + h - 4; // Padding
            baseline = 'bottom';
        } else {
        ...
        // Apply Offset to Text Y
        doc.text(textContent, posX, posY + yOffset, { align: el.align || 'left', baseline: baseline });
        
        if (el.textDecoration === 'underline') {
             const txtWidth = doc.getTextWidth(textContent);
             const lineY = (baseline === 'top' ? posY + Number(el.fontSize) : (baseline === 'bottom' ? posY : posY + Number(el.fontSize)/2)) + 2 + yOffset; 
             let lineX = posX;
