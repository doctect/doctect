    ...
    quizLevels: [
        ...
        { title: 'L3 · server & data', questions: [
            ...
            { q: 'A cloud commit stores…',
              options: ['A diff against the parent', 'A gzip’d full snapshot of AppState', 'Only changed templates', 'A patch file'],
              answer: 1, why: 'Full snapshots (decided pre-code) make history, restore, fork and merge simple reads. stateCodec.js does the gzip.' },
            ...
            { q: 'A merge request’s diff is…',
              options: ['Snapshotted when opened', 'Recomputed live on every view', 'Cached for an hour', 'Computed client-side only'],
              answer: 1, why: 'A request that becomes conflicted after upstream changes is caught on view — and the merge endpoint re-verifies under lock anyway.' },
            ...
            { q: 'Star-rating averages are…',
              options: ['Denormalized onto projects and updated on write', 'Computed at read time with SQL AVG()', 'Computed client-side', 'Cached in Redis'],
              answer: 1, why: 'A live AVG can’t drift the way a hand-maintained counter can — ratings change on every edit and delete.' },
        ...
        { title: 'L5 · war stories', questions: [
            ...
            { q: 'The tag filter’s “exact match” leaked because…',
              options: ['Case sensitivity', 'Unescaped % and _ in the LIKE pattern matched across JSON boundaries', 'Unicode', 'A join bug'],
              answer: 1, why: 'Fixed with an ESCAPE clause behaving identically on Postgres and SQLite, plus a regression test.' },
            ...
            { q: 'Two commits in the same second used to order randomly because…',
              options: ['UUIDv4 sort', 'SQLite CURRENT_TIMESTAMP has whole-second resolution; the tiebreak was a random UUID', 'A race in Express', 'Clock skew'],
              answer: 1, why: 'Fixed by app-generated millisecond timestamps instead of the database default.' },
            ...
            { q: 'The page-dimension unit dropdown (pt/px/in/mm) was, for its whole life before the layers follow-up…',
              options: ['Rounding wrong', 'Purely decorative — the conversion table was imported but never called', 'Metric-only', 'Breaking undo'],
              answer: 1, why: 'Inputs always showed raw points whatever the dropdown claimed. Switching units now re-expresses size with round-trip drift tests.' },
