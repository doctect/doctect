...
- **`components/cloud/CloudMenu.tsx`'s "↳ forked from upstream — view source" link (shown in the editor for a forked cloud project) is deliberately excluded**, even though it's also a link to `/gallery/:id`. Found during the final whole-branch review, after the 4 call sites above were already approved and built. Left as a plain full-page navigation: it navigates *away from the editor's working canvas*, where a modal popping up over the canvas would be a worse fit than it is on the gallery/profile/MR pages (all simple list/detail views already). Not an oversight — confirmed with the project owner.
...
5. **Manual verification in a real browser**: click through from the gallery grid (modal appears over a visibly-dimmed grid, URL updates, back button and Escape both close it), and the "forked from" link from inside an already-open modal (opens the upstream project as a modal without nesting). **On testing the direct-hit fallback specifically:** use a genuinely fresh context — a new tab, an external link, or `page.goto` with no prior navigation — not a same-tab reload. Browsers persist `window.history.state` (where `backgroundLocation` lives) across a same-tab reload, so refreshing the tab the modal is already open in will correctly keep showing the modal, not the full page — that's expected, not a bug, but it means a same-tab refresh doesn't actually exercise the fallback path. (Found during Task 4's real-browser verification.)
