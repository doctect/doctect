...
- **Three disclosed, unresolved findings surfaced during this feature's implementation and final verification**, none treated as blocking (all are narrow/edge-case or pre-existing, not confirmed to have ever affected a real user):
  ...
  - **Direct/hard navigation to any non-root client route 404s when served from the production build** (discovered during this doc's own production build+boot verification — not a gallery-specific issue, and not a regression from this feature set). `server/app.js`'s SPA catch-all calls `res.sendFile(path.join(distPath, 'index.html'))` with no `root` option; under the currently-installed Express 5 / `send` package combination, that specific call form 404s even though the file exists (confirmed: `res.sendFile('index.html', { root: distPath })` resolves correctly instead). This affects a fresh/hard load of *any* non-root path — `/gallery`, `/login`, `/app`, `/mr/:id`, `/u/:username`, etc. — identically, e.g. a bookmark, a shared link, or a browser refresh. Normal in-app use is unaffected: loading `/` and then navigating client-side (clicking the Gallery link, forking, etc.) works correctly, since the SPA's own router handles that without another request to the server. Confirmed pre-existing and unrelated to this plan — the identical `res.sendFile` call already existed, byte-for-byte, in `server/index.js` before this feature set's first commit — and unrelated to `npm run dev` (Vite's dev server has its own correct SPA fallback), which is why no earlier verification in this plan caught it. Not fixed here (out of scope for this task); flagged for follow-up.
