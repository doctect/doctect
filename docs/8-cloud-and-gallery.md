...
- **Better Auth administrator HTTP routes are disabled**: the application retains Better Auth's administrator plugin because sign-in uses its active-ban and expiry enforcement. Admin endpoint `GET`/`POST` requests, plus state-changing methods that reach `/api/auth/admin` or a descendant, receive JSON `404 { "error": "Not found" }` before Better Auth's handler. Valid CORS `OPTIONS` preflights may return `204` earlier, and requests with invalid hosts may return `400` earlier. No external HTTP call reaches the plugin administrator handler; account administration is exposed only through the application-owned routes documented below.
