...
- **Session cookies**, not bearer tokens — better-auth issues an httpOnly session cookie on sign-in. Every `requireAuth`/`optionalAuth` check resolves the cookie through `auth.api.getSession`, then reads current `banned`/`banExpires` fields directly from the user table. If that fresh state is actively suspended, the guard deletes all target sessions and treats the request as unauthenticated (`401` for required auth, `user: null` for optional auth).
...
- Session-suspension trigger behavior is executed against SQLite, and exact PostgreSQL function/trigger statements plus row-lock serialization are contract-tested. Repository has no live PostgreSQL test harness, so migration `012_session_suspension_guard` has not been executed against a live PostgreSQL server in automated verification.
