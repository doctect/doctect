...
- **Three disclosed, unresolved findings surfaced during this feature's implementation and final verification**, none treated as blocking (all are narrow/edge-case or pre-existing, not confirmed to have ever affected a real user):
  - **Preview render/upload error handling in `PublishModal` — and, since the listing-editing round, in `EditListingModal` too** (originally flagged during the publish-wizard work): the render and upload steps share a single `catch` block, so a failure only surfaces as "which phase failed" indirectly (whether preview thumbnails are already showing). More significantly, thumbnail rendering (`generateThumbnails`) only throws on a *fully empty* result — both callers guard with `rendered.length === 0` and neither compares the count against the selection, so a partial render (fewer images produced than pages selected, e.g. from a transient canvas-context allocation failure) succeeds with fewer thumbnails than the user chose and no warning surfaced. On publish that puts a new listing live short of previews. On an edit the consequence is worse: `replaceThumbnails` deletes the existing set before inserting, in the same transaction, so a partial render silently replaces a **live public listing's** previews with fewer images than the owner selected and the originals are gone. (The edit path only re-renders when the owner changes the page list; an untouched list sends no thumbnails and cannot hit this.) Both require either a render-time failure or a state race to trigger and neither has been reproduced in practice, but the gap is real as specified — and the listing editor widened its blast radius.
