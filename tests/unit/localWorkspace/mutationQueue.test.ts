...
describe('deferred project preparation', () => {
  ...
  it('loses authority and rejects pending work when post-operation publication fails', async () => {
    vi.useFakeTimers();
    const firstWrite = deferred<WorkspaceSnapshot>();
    const started = deferred();
    ...
    const durableSnapshot = snapshotWith(projectNamed('Durable first'));
    vi.spyOn(globalThis, 'structuredClone').mockImplementationOnce(() => {
      throw new DOMException('Publication clone failed.', 'DataCloneError');
    });
    firstWrite.resolve(durableSnapshot);

    const [firstError, pendingError] = await outcomes;
    expect(firstError).toBe(pendingError);
    expect(firstError).toMatchObject({ code: 'authority-lost' });
    expect(saveProject).toHaveBeenCalledOnce();
