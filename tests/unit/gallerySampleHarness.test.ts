...
return {
  ...
  page: { id: 'page', name: 'Page', width: RM_PP_WIDTH, height: RM_PP_HEIGHT, elements: [
    ...
    { id: 'page_grid', type: 'grid', x: 20, y: 60, w: 100, h: 30, stroke: '', strokeWidth: 0, gridConfig: { cols: 2, gapX: 8, gapY: 8, sourceType: 'current', displayField: 'title', gridBorderMode: 'all', gridBorderColor: '#888888', gridBorderWidth: 1, gridBorderStyle: 'solid' } }
  ]}
};`;
...
describe('gallery sample harness', () => {
    ...
    it.each([
        ['all', 0, 'solid', 'positive finite'],
        ['outside', Number.POSITIVE_INFINITY, 'solid', 'positive finite'],
        ['inside', 1, 'none', "must not be 'none'"],
    ])('requires usable borders for %s grid mode', (mode, width, style, error) => {
        const sample = execute();
        const grid = sample.templates.page.elements.find((element: any) => element.id === 'page_grid');
        Object.assign(grid.gridConfig, {
            gridBorderMode: mode,
            gridBorderWidth: width,
    ...
    it.each([
        ['element stroke', { stroke: '#123456' }],
        ['element stroke width', { strokeWidth: 1 }],
    ])('rejects grid %s alongside gridConfig borders', (_, elementStyle) => {
        const sample = execute();
        Object.assign(sample.templates.page.elements.find((element: any) => element.id === 'page_grid'), elementStyle);

        expect(validateSharedGalleryInvariants(sample)).toEqual(expect.arrayContaining([
            expect.stringContaining('page_grid grid element stroke must be empty or none with strokeWidth 0'),
        ]));
    });
    ...
    it('rejects hierarchy node keys and IDs generated by Math.random', () => {
        const randomHierarchy = hierarchySource
            .replace('const nodes = {', "const randomId = `random_${Math.random()}`;\nconst nodes = {")
            .replace("blank_workspace: { id: 'blank_workspace'", "[randomId]: { id: randomId");

        expect(validateSharedGalleryInvariants(execute(templatesSource, randomHierarchy))).toEqual(expect.arrayContaining([
            expect.stringContaining('hierarchy node keys are not deterministic across repeated execution'),
            expect.stringContaining('hierarchy node IDs are not deterministic across repeated execution'),
        ]));
    });

    it('accepts hierarchy node keys and IDs made by injected createId', () => {
        const deterministicHierarchy = hierarchySource
            .replace('const nodes = {', "const generatedId = createId('blank');\nconst nodes = {")
            .replaceAll("'blank_workspace'", 'generatedId')
            .replace('blank_workspace:', '[generatedId]:');

        const errors = validateSharedGalleryInvariants(execute(templatesSource, deterministicHierarchy));
        expect(errors.filter(error => error.includes('not deterministic across repeated execution'))).toEqual([]);
    });
