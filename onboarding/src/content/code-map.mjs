...
export const CODE_MAP = {
    ...
    deepDives: [
        ...
        { id: 'validate-appstate', title: 'validateAppState — the one gate every stored AppState passes',
          tagline: 'Create, commit, merge: three call sites, one structural validator.',
          sections: [
            { text: 'Shape checks, a 5 MB cap, and node/element count caps — enforced before anything is stored. It has exactly three call sites, one per path that accepts an AppState from a client: project create, commits, and the merge endpoint’s produced result (a merge that validates badly refuses rather than writing). Publish is deliberately NOT one of them: it takes listing fields and an If-Match head, never a state, and pins a commit that was already validated on the way in.', anchorId: 'validate-appstate' },
