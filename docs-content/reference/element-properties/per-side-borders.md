...
**Per-side borders replace the uniform Stroke — they don't add to it.** The instant Customize is clicked, Stroke stops drawing anything by itself; a side you leave off shows *no* border, not a fallback to whatever Stroke held. **On a rectangle or text box, keep Stroke's own width above zero even after customizing:** on export, the PDF gates those elements' per-side borders behind the global Stroke width, so a border that looks fine on the canvas silently vanishes from the PDF the moment Stroke's width drops to 0. (Grid cell borders don't share this gate.)
