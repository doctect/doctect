...
**Save to cloud** (the cloud icon, top-right of the editor) sends the whole project to the server as one deliberate snapshot — it is *not* sync. Nothing watches your editing or uploads in the background, and closing the tab sends nothing anywhere; the project persists locally in IndexedDB through `LocalWorkspaceStore` before and after each cloud save. Each save asks for a [commit](/docs/reference/commit) message and stacks a new immutable commit, advancing the project's **HEAD** pointer. The first save on a fresh project reads **Save to cloud (new)**; once that cloud record exists the menu grows [Version history](/docs/reference/version-history-restore) and Publish.
