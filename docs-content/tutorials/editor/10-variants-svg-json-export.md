...
> [!NOTE]
> Custom presets persist locally in IndexedDB through `LocalWorkspaceStore`: they survive reloads, but they don't follow you to another machine. Clearing site data still removes this local data. To move a design between machines or people, export the project itself as JSON — the inspector's Text mode, or the download offered when closing a tab — and import it on the other side.
