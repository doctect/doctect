...
Switch a grid's Source dropdown to **Children of Specific Page...** and a dashed **Select Page...** button appears underneath it. Click it, and the **Select Source Node** modal opens: a node tree starting at the root, the same shape as the Hierarchy sidebar you've been clicking through since Canvas Basics, just in a popup so you can pick a target without leaving whichever page you're actually editing.
...
Three fields decide layout and content, all in the same Grid Configuration section: **Cols** (cells per row before wrapping to a new one), **Gap X** and **Gap Y** (pixel spacing between columns and between rows, set independently), and further down, **Display Template** (what text lands inside each cell).

Display Template starts blank, which behaves exactly like typing `{{title}}` — the same default Data Binding covered for ordinary text boxes. Type a bare field name with no braces at all — `day_num`, say — and PDF Architect wraps it in `{{ }}` automatically; type a full template with literal characters and more than one placeholder (`{{month_short}} {{day_num}}`) and that works too. A row of quick-insert buttons underneath lists every field it can find across the first 20 resolved children — `title` always included — so the braces rarely need typing by hand at all.
...
Whichever mode is picked, the **Cell Border** row underneath sets the actual look: a color swatch, a width, a style (Solid, Dashed, Dotted, Double, or None), and a corner radius — all four grid-specific, defaulting from the element's own Stroke / Stroke Width / Border Style but never writing back to them. A width of 0 — the default on a freshly-drawn grid — means no cell border shows no matter which mode is selected; Cell Border Mode only ever decides *where* a border could go, not whether one actually exists. One more toggle, **Empty Cell Borders** (off by default), draws the border for slots with no real cell in them at all, like the leading gap before day 1 in an offset calendar — leave it off and those slots stay invisible.
...
| --- | --- | --- |
| **Header Row** | Row 0 — whichever real cells land in the grid's first row | Fill, text color, Bold |
| **First Column** | Column 0 — whichever real cells land in the grid's first column | Fill, text color, Bold |
| **Alternate Rows** | Every other data row | One fill, for the odd rows |
