...
**Fill** is a color swatch, a small **✕** next to it that clears the fill back to none, and a dropdown: **Solid Color** or **Pattern**.

Switch it to Pattern and two more controls appear: a dropdown offering **Horizontal Lines**, **Vertical Lines**, or **Dots**, and two number fields beside it, **Gap** and **Weight**. Gap sets the distance between repeats; Weight sets line thickness for the two line patterns, or dot diameter for Dots.
...
**Stroke**, right below Fill, is the same swatch-plus-✕ shape, followed by a width field and a style dropdown: **Solid**, **Dashed**, **Dotted**, **Double**, or **None**.
...
**Auto width**, at the top, only shows for a text-only selection. Switch it on and the box resizes to fit whatever's typed, growing and shrinking as you edit it; switch it off and it goes back to a fixed size again.

Below the text box itself sits the font row — a searchable **font family** picker (click it, then type to filter a long list), a **size** field in points, and a **text color** swatch — then three independent toggle buttons: **Bold**, *Italic*, and **Underline**. There's a fourth text-decoration value in the underlying format, strikethrough, but nothing in this row sets it; it only ever arrives through a hand-written generator script.

Next, **Overflow** — what happens when text doesn't fit a fixed-size box:
...
**Wrap**, the checkbox right beside it, turns line-wrapping on or off; off, everything stays on one line no matter how wide the box is. Both controls only mean something for a fixed-size box — turn Auto width on and they grey out, with the panel explaining why in a tooltip, since a box that resizes to its own content never has anything left over to overflow.

Align and vertical align share the next row, split by a thin divider: three buttons for horizontal (Left, Center, Right), three for vertical (Top, Middle, Bottom). These control only how text sits inside its own box — don't confuse them with the toolbar's alignment buttons, later in this tutorial, which move whole shapes relative to each other and happen to reuse very similar icons. A freshly drawn text box starts out Center/Middle, not the Left/Top you might expect, unless a previous text box in this same browser already set something else — every control on this row, and the font row above it, remembers your last choice for the next text box you draw.

**Padding**, at the bottom, is four number fields, Top, Right, Bottom, Left, with a link icon that — on by default — keeps all four moving together whenever you change one. It's disabled under the exact same condition as Overflow and Wrap: Auto width has to be off first.
...
Rotation always turns a shape around its transform origin — the same pivot point you dragged in [Canvas Basics](/docs/editor/canvas-basics) to move a shape's center of rotation off-center. That point is stored as a pair of normalized coordinates, 0 to 1 across the shape's own width and height, defaulting to dead center, `{0.5, 0.5}`, until the pivot handle is dragged somewhere else. There's no numeric field for it in the panel — the canvas handle is the only control.

Z-Index breaks ties within a single layer only: it decides stacking order among elements that share a layer, not across the whole page. Layers themselves, and how they interact with Z-Index, are their own tutorial later in this track.

**Opacity** and **Radius** share the row just below Per-Side Borders: Opacity is a slider from 0 to 1 (1 is fully opaque), and Radius is a plain number that rounds a rectangle's corners.
