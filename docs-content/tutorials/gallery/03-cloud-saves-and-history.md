...
Two more items appear conditionally. Once the project *is* published, **Edit gallery listing…** joins the list — it changes the public description, tags, and preview pages without publishing anything new, and the publishing tutorial covers it under [Fixing a listing after the fact](/docs/gallery/publishing#fixing-a-listing-after-the-fact). And on projects you *forked* from someone else's gallery project, a small **↳ forked from upstream — view source** link back to the original appears, plus **Propose changes to upstream…** for sending your edits back as a merge request. Forks and merge requests get their own tutorials later in this track — for now, just know the menu will tell you when a project has an upstream.
...
![The My projects page: storage usage bar, then one row per cloud project with visibility, size, version count, an Edit listing action on the public row, and a delete button](/docs-assets/gallery/my-projects.png "Every cloud project you own — public or private — with the storage meter on top")

Each row is one cloud project: its name, a **public** or **private** badge (private is the default — publishing is what flips it), its size, how many versions it holds, and when it was last updated. This list is the full inventory of what the cloud holds under your name — projects you've only ever kept local don't appear, because the cloud has never heard of them. Public rows carry one extra action, **Edit listing**, which opens [the gallery listing editor](/docs/gallery/publishing#fixing-a-listing-after-the-fact) for that project.
